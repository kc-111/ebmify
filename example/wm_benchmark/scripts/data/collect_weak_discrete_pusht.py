"""Forked from ``upstream_scripts/data/collect_weak_discrete_pusht.py``
— sharded ``WeakPolicy`` runs on ``swm/PushT-Discrete-v1``.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    traj_per_shard = ns.num_traj // ns.num_shards
    rng = np.random.default_rng(ns.seed)
    out_root = datasets_root(ns.cache_dir) / (ns.out_name or "pusht_discrete_weak_100")
    out_root.mkdir(parents=True, exist_ok=True)

    for i in range(ns.num_shards):
        out = out_root / f"shard_{i}.lance"
        world.collect(out, episodes=traj_per_shard,
                      seed=rng.integers(0, 1_000_000).item())
        print(f"[collect_weak_discrete_pusht] wrote {out}")
    return 0
