"""Forked from ``upstream_scripts/data/collect_pointmaze.py`` — random
trajectories on ``swm/OGBMaze-v0`` with ``loco_env_type='point'``.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    out = datasets_root(ns.cache_dir) / f"{ns.out_name or 'pointmaze-teleport-navigate-v0'}.lance"
    world.collect(path=out, episodes=ns.num_traj, seed=ns.seed)
    print(f"[collect_pointmaze] wrote {out}")
    return 0
