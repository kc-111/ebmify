"""Threshold OOD classifier on torchvision ImageNet-pretrained ResNet50 features.

Proper inductive protocol:
 1. Build the kernel from ``cifar10 train`` only (Gram = Z_train).
 2. Tune the RFF bandwidth on ``cifar10 test`` as a held-out validation set:
    pick the smallest ell at which the validation leverage is statistically
    indistinguishable from the training leverage (AUROC <= ``--tune-target``).
    This is honest -- test is only used to pick a hyperparameter, never
    placed in the Gram.
 3. Set the OOD threshold to the 0.95 quantile of h on the training Gram.
 4. Score every source under three phi maps: {z, RFF(z), [z; RFF(z)]}.

``z = ResNet50.avgpool_features(x)``, 2048-dim, post-avgpool, pre-FC,
with cifar images bilinearly upsampled to 224x224 and normalized with
ImageNet statistics.

Usage:
    python example/cifar/cifar_resnet_ood_threshold.py --dataset cifar10
"""
...
def make_feature_extractor(device: str) -> nn.Module:
...
def encode(model: nn.Module, x: torch.Tensor, *,
    ...
    """Encode (B, 3, 32, 32) in [0,1] -> (B, 2048) ResNet50 penultimate features."""
...
def auroc(s_pos: np.ndarray, s_neg: np.ndarray) -> float:
    """Mann-Whitney rank-based AUROC. Positive = high h."""
...
def roc_auroc(s_pos: np.ndarray, s_neg: np.ndarray) -> tuple[np.ndarray, np.ndarray, float]:
    """Sweep threshold; return (fpr, tpr, AUROC)."""
...
def tune_bandwidth(Z_train: torch.Tensor, Z_val: torch.Tensor, *,
    ...
    """Pick the smallest ell with AUROC(val, train) <= target_auroc.

    Tunes the RFF bandwidth on a held-out ID validation set so that the
    kernel treats it as in-sample. No OOD data used.
    """
    sweep = []
...
def main() -> None:
    ...
    ap.add_argument("--batch", type=int, default=256, dest="batch_size",
                    help="Batch size for ResNet50 forward.")
    ...
    ap.add_argument("--n-ell", type=int, default=24,
                    help="Number of ell values in the tuning sweep.")
    ap.add_argument("--tune-target", type=float, default=0.55,
                    help="Pick the smallest ell with AUROC(val,train) <= this.")
    ...
    model = make_feature_extractor(device)
    z_dim = 2048
    print(f"  feature dim = {z_dim}")

    # --- Load + encode all data through ResNet50 -------------------------
    ...
    print("encoding all x sources through ResNet50 ...")
    ...
    # cifar10 test is the held-out validation for bandwidth tuning.
    test_src_idx = 1
    ...
    # --- Compute leverage on Gram = Z_train only (Setup A) ---------------
    specs = [
        ("phi = z",           lambda z: z),
    ...
    # --- Print tables ----------------------------------------------------
    print(f"\n=== Gram = {args.dataset} train  (ell* = {ell_star:.3f}) ===")
    ...
    # --- Plot: tuning sweep (top) + histograms (mid) + ROC (bottom) -----
    if args.no_plot:
        return
    ...
    if sweep is not None:
        ...
        ax.set_ylabel("AUROC")
        ax.set_title(
            f"Bandwidth tuning on held-out cifar10 test  "
            f"(want AUROC ~ 0.5)",
            fontsize=10,
        )
