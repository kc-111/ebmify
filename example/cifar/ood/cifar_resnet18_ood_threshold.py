"""Threshold OOD classifier on features from a CIFAR-10-supervised ResNet18.

Same protocol as ``cifar_dinov2_ood_threshold.py``:
 1. Build the kernel from cifar10 train only (Gram = Z_train).
 2. Tune the RFF bandwidth on cifar10 test as a held-out validation
    set (smallest ell with AUROC(val, train) <= --tune-target).
 3. Threshold = 0.95 quantile of h(z) on the train Gram.
 4. Score under three phi maps: {z, RFF(z), [z; RFF(z)]}.

Compared to the pretrained DINOv2/ResNet50 scripts, the feature
extractor here is the locally trained 512-d penultimate of a
ResNet18 supervisedly fit on cifar10 -- a "trained from scratch on the
in-data" comparison point.

Usage:
    python example/cifar/ood/cifar_resnet18_ood_threshold.py
"""
...
from cifar_resnet18_train import (  # noqa: E402
    CIFAR_MEAN, CIFAR_STD, make_resnet18_cifar, resnet18_ckpt_path,
)
...
def load_trained_resnet18(ckpt_path: Path, device: str) -> tuple[nn.Module, int]:
    """Load the CIFAR-trained ResNet18 with fc swapped for Identity.

    Returns ``(model, z_dim=512)``. The penultimate features are what the
    classifier head sees, so they encode whatever the supervised loss
    pushed the network to disentangle.
    """
...
def encode(model: nn.Module, x: torch.Tensor, *,
    ...
    """Encode (B, 3, 32, 32) in [0,1] -> (B, 512) penultimate features."""
...
def best_balanced_accuracy(s_pos: np.ndarray, s_neg: np.ndarray,
                           ) -> tuple[float, float]:
    """Argmax balanced accuracy = (TPR + TNR)/2 over thresholds.

    Equivalent to balanced classification on equal-prior ID vs OOD.
    Returns (best_balanced_acc, threshold_at_argmax).
    """
...
def main() -> None:
    ...
    ap.add_argument("--ckpt", type=str, default="",
                    help="Path to ResNet18 checkpoint; defaults to the cache.")
    ap.add_argument("--tag", type=str, default="",
                    help="Tag used at training time (same --tag).")
    ...
    if not ckpt_path.exists():
        raise FileNotFoundError(
            f"No checkpoint at {ckpt_path}. Run cifar_resnet18_train.py "
    ...
    print("encoding all x sources through ResNet18 ...")
    ...
    print(f"\n=== Gram = {dataset} train  (ResNet18-CIFAR, ell* = {ell_star:.3f}) ===")
    ...
    # Balanced accuracy at the threshold that maximizes (TPR+TNR)/2.
    # Skips the cifar10 (in-data) row since that is the baseline itself.
    print(f"\nBalanced accuracy (best over tau) vs '{base_label}' baseline:")
    print(f"  -- each cell is max_t [(TPR + TNR)/2]; baseline n={len(per_phi[0]['baseline'])}, "
          f"source n={len(per_phi[0]['rows'][0][1])}")
    head = f"  {'phi':<22} {'D':>5}"
    for name, _, _ in z_sources:
        head += f"  {name:>13}"
    print(head)
    for entry in per_phi:
        ...
        for src_name, h, _ in entry["rows"]:
            if src_name == in_name:
                row_str += f"  {'-':>13}"
                continue
            bacc, _ = best_balanced_accuracy(h, entry["baseline"])
            row_str += f"  {bacc:>13.4f}"
        print(row_str)
    ...
    fig.suptitle(
        f"ResNet18-CIFAR (supervised, from scratch) OOD on {in_name}:{norm_tag} "
    ...
    out = (REPO_ROOT / "example" / "out" / "cifar"
           / f"cifar10_resnet18{suffix}{norm_suffix}_ood_threshold.png")
